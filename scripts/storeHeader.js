...
var getAbi = helpers.getAbi;
var getHex = helpers.getHex;
...
module.exports = function (callback) {
    ...
    var contract = web3.eth.contract(getAbi());
    var btcrelay = contract.at(address);

    // store a parent header (can only be called once)
    block100kPrev = '0x000000000002d01c1fccc21636b607dfd930d31d01c3a62104612a1719011250'

    // store multiple block headers
    var headers = [
    ...
    setParent = async (blockHash) => {
        let result = await btcrelay.setInitialParent(blockHash, 99999, 1, {from: web3.eth.accounts[0]});
        let transaction = web3.eth.getTransactionReceipt(result);
        if (transaction != 0) {
            console.log("Stored parent");
            helpers.eventPrint(transaction);
        } else {
            console.log("error storing parent");
            console.log(transaction);
        }
    }

    addHeader = async (header) => {
        // var header = helpers.convertHex(rawheader);
        // console.log(header);
    ...
    setParent(block100kPrev).then(function (result) {
        if (result != 0) {
            for (var i = 0; i < headers.length ; i++) {
                var buffer = Buffer.from(headers[i], 'hex');
                var header = [...buffer];
                addHeader(header);
            }
        } else {
