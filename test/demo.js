import eventFired from './helpers';
...
contract('ERCXXX_BTCRelay', async (accounts) => {
    ...
    it("Redeem BTC-ERC from Bob", async () => {
        ...
        // check if issue event is fired
        let issue_register_col_tx = await btc_erc.registerIssue(amount, { from: bob, value: web3.toWei(amount, "ether") });
        eventFired(issue_register_col_tx, "RegisterIssue");

        let issue_col_tx = await btc_erc.issueCol(bob, amount, btc_tx, { from: relayer, gas: gas_limit });
        eventFired(issue_col_tx, "Issue");

        // check if Bob's balance is updated
        balance_bob = await btc_erc.balanceOf.call(alice);
        balance_bob = balance_alice.toNumber();
        assert.equal(balance_alice, amount, "SUCCESS COL: Bob balance should be 0.01");
